...
router.post('/', function (req, res, next) {
    ...
    if (req.body.career_name == "" || req.body.career_birth == "" || req.body.career_start == "" || req.body.career_end == "" || req.body.career_department == "" || req.body.career_rank == "") {
        ...
        conn2.connect((err) => {
            ...
            const career_key = XSS_Check(req.body.career_key);
            ...
            const sql = `insert into certchain_career(from_no, to_no, name, birth, date_join, date_leave, dept, rank) values((select no from certchain_account where email=?), (select account_no from certchain_key_encoded where encoded_key=?), ?, ?, ?, ?, ?, ?)`;
